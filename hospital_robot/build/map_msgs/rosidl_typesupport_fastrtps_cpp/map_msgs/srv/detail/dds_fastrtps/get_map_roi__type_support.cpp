...
#include "map_msgs/srv/detail/get_map_roi__rosidl_typesupport_fastrtps_cpp.hpp"
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = map_msgs::srv::GetMapROI_Request;
    is_plain =
      (
...
static size_t _GetMapROI_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_GetMapROI_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = map_msgs::srv::GetMapROI_Response;
    is_plain =
      (
...
static size_t _GetMapROI_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_GetMapROI_Response(full_bounded, is_plain, 0);
