...
#include "map_msgs/srv/detail/get_map_roi__rosidl_typesupport_fastrtps_c.h"
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = map_msgs__srv__GetMapROI_Request;
    is_plain =
      (
      offsetof(DataType, l_y) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _GetMapROI_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_map_msgs__srv__GetMapROI_Request(
...
#include "nav_msgs/msg/detail/occupancy_grid__functions.h"  // sub_map
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = map_msgs__srv__GetMapROI_Response;
    is_plain =
      (
...
static size_t _GetMapROI_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_map_msgs__srv__GetMapROI_Response(
    full_bounded, is_plain, 0);
