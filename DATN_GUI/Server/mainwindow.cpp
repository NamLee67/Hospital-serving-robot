...
void MainWindow::seupServer()
{
    ...
    connect(_server, &ServerManager::waitNumberUpdated, this, &MainWindow::onWaitNumberUpdated);
...
void MainWindow::onWaitNumberUpdated(int number)
{
    qDebug() << "MainWindow received wait number:" << number;
//    ui->lstClients->addItem(QString("Client disconnected: %1").arg(id));
    ui->lstNumbers->addItem(QString("STT : %1").arg(number));
}
